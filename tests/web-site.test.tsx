...
test("web build publishes catalog assets and lets eikon paths hit the filesystem", async () => {
  ...
  expect(html).toContain('href="https://fonts.googleapis.com/css2?family=Cascadia+Code:ital,wght@0,200..700;1,200..700&display=swap"')
...
test("web preview uses terminal-like cells, square cards, mobile drawer, and smooth timing", async () => {
  ...
  expect(css).toContain('--mono: "Cascadia Code", ui-monospace')
  expect(css).toContain("font-family: var(--mono)")
  expect(css).toContain("--terminal-font: var(--mono)")
  ...
  expect(css).not.toContain("previewMeta")
