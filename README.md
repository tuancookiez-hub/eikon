...
catalog/                packed .eikon files + index.json  — what plays
  <name>.eikon          header carries source_url → eikons/<name>/
  index.json            generated: {name, author, glyph, w, h, source, poster}
eikons/                source media                       — what Studio edits
  <name>/
...
into the header (`<raw>/eikons/<name>/manifest.json` + referenced files).
...
```sh
# 1. per-state mp4s → eikons/<name>/states/<state>/{loop,start}.mp4
#    layout + motion constraints: docs/VIDEO_HANDOFF.md
# 2. pack
bun src/cli.tsx pack eikons/<name>/states <name>.eikon --name <name> --glyph ◆
...
PR to `eikons/<name>/`. CI runs `eikon lint` on both the manifest and any
